...
import { deepToolContent } from "./tool-deep-content"
...
const openings = (title, category) => [
  `When you need reliable results for ${title.toLowerCase()}, working inside the browser keeps your workflow simple. AllInOneTools focuses on clarity: you open the tool, complete your task, and move on without installing desktop software or creating an account. That helps students, freelancers, and teams who switch devices often.`,
...
const midBlocks = {
  ...
  'PDF Tools': (title, seed) => [
    `PDF remains the standard for contracts, invoices, reports, and printable coursework because layout stays stable across operating systems. ${title} on AllInOneTools is designed for quick conversions from office formats or web content into shareable PDFs. When you send a file to clients or teachers, a consistent PDF reduces “it looks different on my screen” confusion.`,
  ...
  'Other Tools': (title, seed) => [
    ...
    `Document outputs with timestamps and context when you share results with colleagues. For DNS or barcode-related tasks, a screenshot or exported file plus a short note avoids misunderstandings later. AllInOneTools processes your input for the task at hand and does not retain your data after the session completes.`,
  ],
...
const privacy = (title) => [
  `AllInOneTools processes files and inputs to deliver ${title} results and does not use your content for training unrelated models or for resale. Avoid uploading highly confidential material on any shared or public network; when in doubt, use offline tools approved by your organization. Clearing your browser after sensitive sessions adds another layer of caution on shared machines.`,
...
function buildFaqs(tool, seed) {
  ...
  const q1 = pickVariant(seed, 0, [
    ...
    `What problems does ${t} solve on AllInOneTools?`,
  ])
  ...
  const q2 = pickVariant(seed, 1, [`Is ${t} free on AllInOneTools?`, `Does ${t} require payment or a subscription?`, `Are there hidden fees for ${t}?`])
  const a2 = pickVariant(seed, 1, [
    `Yes—${t} is free at the point of use, with no mandatory account. AllInOneTools is funded in part by display advertising so we can keep utilities available to students and small businesses worldwide. Fair-use limits and our terms still apply; abusive automation or resale of the service itself is not OK.`,
  ...
  const q4 = pickVariant(seed, 3, [
    `How does AllInOneTools handle privacy for ${t}?`,
  ...
  const q7 = pickVariant(seed, 6, [
    `Which other AllInOneTools utilities pair with ${t}?`,
...
export function buildToolSeoContent(tool) {
  ...
  const { title, description, category, id } = tool
  ...
  let deep = deepToolContent[id]
  if (deep && deep.copyFrom) {
    deep = deepToolContent[deep.copyFrom]
  }
  ...
  return {
    intro: {
      ...
      lead: `${title} is a ${category.toLowerCase()} utility on AllInOneTools. Read the short summary once, then use the sections below for workflows and FAQs—we do not repeat the same description block in every paragraph.`,
      summary,
      paragraphs: [open, open2],
    },
    sections,
    benefitCards,
    faqs: deep?.faqs ? deep.faqs : buildFaqs(tool, seed),
    relatedNote: `Explore more ${category} tools from the sidebar to build an end-to-end workflow around ${title.toLowerCase()} without leaving AllInOneTools.`,
    deepContent: deep ? {
